...
Чат для общин. Как через `!` вы отправляете сообщения в глобальный чат, через `@` вы отправляете сообщение в чат общины. Чтобы выбрать общину, в чат которой вы отправляете сообщение, напишите `/teamschat <link>`. Строка `link` это сылка, которую Глава Общины прописал в строке Сылка.
...
`/map hide`, чтобы скрыть себя на карте, `/map show`, чтобы показать
